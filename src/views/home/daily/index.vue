    ...
    <el-table v-loading="loading" :data="dailyList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="类型" align="center" prop="type">
        <template #default="scope">
          <dict-tag :options="home_daily_type" :value="scope.row.type"/>
        </template>
      </el-table-column>
...
import { listDaily, getDaily, delDaily, addDaily, updateDaily } from "@/api/home/daily";

const { proxy } = getCurrentInstance();
const { home_daily_type } = proxy.useDict('home_daily_type');
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除日常记事编号为"' + ids + '"的数据项？').then(function() {
    return delDaily(ids);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('home/daily/export', {
    ...queryParams.value
